...
async def search_contacts(field: SearchField,
    skip: int = 0, limit: int = 100, query: str = None, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)
):
    contact_service = ContactService(db)
    contacts = await contact_service.search_contacts(search_field=field, query=query, skip=skip, limit=limit, user=user)
    return contacts
...
async def update_status_contact(
    body: ContactStatusUpdate, contact_id: int, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)
):
    contact_service = ContactService(db)
